<div class="mx-auto max-w-2xl" x-data="{ tab: 'topup' }">
    <div class="mb-4 flex items-center justify-between gap-3">
        ...
        <div class="relative" x-data="{ open: false }">
            <button type="button" x-on:click="open = !open" x-on:click.outside="open = false"
                    class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700 dark:border-[var(--brand-card-border-dark)] dark:bg-[var(--brand-card-dark)] dark:text-slate-200">
            ...
            <div x-show="open" x-cloak x-transition
                ...
                @foreach ($currencyOptions as $code => $meta)
                    <button type="button" wire:key="cur-{{ $code }}" x-on:click="open = false" wire:click="setCurrency('{{ $code }}')"
                            @class([
                                'flex w-full items-center justify-between px-3 py-2 text-left text-xs hover:bg-slate-50 dark:hover:bg-[var(--brand-card-hover-dark)]',
                                'font-bold text-primary' => $displayCurrency === $code,
    ...
    @if ($pendingTopUp)
        <div wire:poll.5s="checkPendingTopUp"
             class="mb-4 flex items-center gap-3 rounded-2xl border border-primary/20 bg-primary/5 p-4 dark:border-primary/20 dark:bg-primary/10">
            <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary/15 text-primary dark:bg-primary/20">
    ...
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-navy via-primary-dark to-primary p-6 shadow-lg shadow-primary/20">
        ...
        <div class="relative">
            <p class="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-white/80">
            ...
            @if ($usdLocal)
                <p class="mt-1 text-xs text-white/70">≈ {{ $usdLocal }} · live rate</p>
            @endif
            ...
            <div class="mt-5 flex items-center justify-between rounded-2xl bg-white/10 px-4 py-3 backdrop-blur">
                <span class="flex items-center gap-2 text-sm text-white/90"><x-icon name="wallet" class="h-4 w-4" /> ≈ NGN</span>
            ...
            @if ((float) $wallet->ngn_balance > 0)
                <div class="mt-2 flex items-center justify-between rounded-2xl bg-white/5 px-4 py-2.5 text-xs text-white/70">
    ...
    <div class="mt-6 inline-flex w-full rounded-full border border-slate-200 bg-slate-100 p-1 dark:border-[var(--brand-card-border-dark)] dark:bg-[var(--brand-card-dark)]">
        <button type="button" @click="tab = 'topup'"
                :class="tab === 'topup' ? 'bg-white text-primary shadow-sm dark:bg-[var(--brand-card-dark)]' : 'text-slate-500 dark:text-slate-400'"
                class="flex-1 rounded-full px-3 py-2 text-sm font-semibold transition">Top Up</button>
        <button type="button" @click="tab = 'payout'"
                :class="tab === 'payout' ? 'bg-white text-primary shadow-sm dark:bg-[var(--brand-card-dark)]' : 'text-slate-500 dark:text-slate-400'"
            ...
            <form wire:submit="topUp" class="space-y-4">
                ...
                <div>
                    ...
                    <select wire:model.live="currency"
                            class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-900 focus:border-primary focus:ring-2 focus:ring-primary/40 dark:border-[var(--brand-card-border-dark)] dark:bg-[var(--brand-card-dark)] dark:text-slate-100">
    ...
    <div x-show="tab === 'spending'" x-cloak class="mt-5">
        ...
        <div class="nx-aurora">
            ...
            <div class="relative grid gap-6 sm:grid-cols-2">
                <div>
                    <p class="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-white/90">
                    ...
                    <p class="mt-1 text-xs text-white/80">spent on eSIMs &amp; numbers this month</p>

                    <dl class="mt-5 space-y-1.5 text-sm">
                        <div class="flex items-center justify-between gap-4">
                            <dt class="flex items-center gap-1.5 text-white/80"><x-icon name="zap" class="h-3.5 w-3.5 text-accent" /> Topped up (USD)</dt>
                            <dd class="font-semibold text-white">${{ number_format($topupUsd, 2) }}</dd>
                        </div>
                        <div class="flex items-center justify-between gap-4">
                            <dt class="flex items-center gap-1.5 text-white/80"><x-icon name="zap" class="h-3.5 w-3.5 text-accent" /> Topped up (NGN)</dt>
                            <dd class="font-semibold text-white">NGN {{ number_format($topupNgn, 2) }}</dd>
                        </div>
                    </dl>
                </div>
                <div class="flex flex-col justify-end">
                    <p class="mb-2 text-right text-[11px] font-medium uppercase tracking-wider text-white/70">Last 14 days</p>
                    <div class="rounded-2xl bg-white/10 p-3 backdrop-blur">
                        @if ($hasSpendData)
                            {{-- text-accent + currentColor (not a literal hex): the sparkline
                                 always tracks the active theme's accent colour, on the master
                                 and on either white-label fork. --}}
                            <svg viewBox="0 0 200 48" class="h-16 w-full text-accent" role="img" aria-label="Daily spending, last 14 days" preserveAspectRatio="none">
                                <defs>
                                ...
                                <polyline points="{{ $sparkline }}" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                        @else
                            <div class="flex h-16 items-center justify-center gap-2 text-xs text-white/70">
