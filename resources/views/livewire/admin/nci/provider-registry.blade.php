                        ...
                        @foreach ($group['rows'] as $r)
                            <tr wire:key="row-{{ $familyKey }}-{{ $r['provider_key'] }}" class="hover:bg-slate-50 dark:hover:bg-[#243352]/40">
                                ...
                                <td class="px-3 py-2.5">
                                    ...
                                    @if ($r['paused_at'])
                                        <span class="ml-1 inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-500 dark:bg-white/10 dark:text-slate-400" title="Paused {{ $r['paused_at']->diffForHumans() }}">
                                            <x-icon name="moon" class="h-2.5 w-2.5" /> Paused
                                        </span>
                                    @endif
                                </td>
