...
@if ($order)
    <div class="py-4 text-center">
        ...
        <p class="text-lg font-bold text-slate-900 dark:text-white">{{ __('numbers.rent.reserved_title') }}</p>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('numbers.rent.reserved_body') }}</p>
        ...
        <div class="mt-5 flex gap-2">
            <button type="button" wire:click="reset_" class="flex-1 rounded-xl border border-slate-200 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 dark:border-white/10 dark:text-slate-300 dark:hover:bg-white/5">{{ __('numbers.rent.rent_another') }}</button>
...
@else
    ...
    <div class="mb-4 inline-flex w-full rounded-full border border-slate-200 bg-slate-100 p-1 dark:border-white/10 dark:bg-white/5">
        <button type="button" wire:click="$set('service', 'whatsapp')"
                class="flex-1 rounded-full px-4 py-1.5 text-sm font-semibold transition {{ ! $isAny ? 'bg-white text-primary shadow-sm dark:bg-[#243352] dark:text-teal-300' : 'text-slate-500 dark:text-slate-400' }}">{{ __('numbers.rent.single_service') }}</button>
        <button type="button" @if ($fullRentAvailable) wire:click="$set('service', '{{ \App\Services\SMS\NumberRequest::SERVICE_ANY }}')" @else disabled @endif
            ...
            {{ __('numbers.rent.any_service') }} @unless ($fullRentAvailable)<span class="text-[10px]">{{ __('numbers.rent.soon') }}</span>@endunless
        </button>
    </div>
    ...
    <div class="space-y-3">
        ...
        @else
            <div class="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-sm text-slate-600 dark:border-teal-400/20 dark:bg-teal-500/5 dark:text-slate-300">
                ...
                {!! __('numbers.rent.any_service_note', ['bold' => '<b>'.__('numbers.rent.any_service_bold').'</b>']) !!}
            </div>
        @endunless
    </div>
    ...
    @if ($isUsRental)
        <div class="mt-4">
            <p class="mb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('numbers.rent.rental_length') }}</p>
            ...
            <label class="mt-3 flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                ...
                {{ __('numbers.rent.auto_renew') }}
            </label>
        </div>
    @else
        <p class="mt-3 rounded-xl bg-slate-50 px-3 py-2 text-xs text-slate-500 dark:bg-white/5 dark:text-slate-400">
            <x-icon name="info" class="mr-1 inline h-3.5 w-3.5" /> {{ __('numbers.rent.short_term_note') }}
        </p>
    @endif
    ...
    <div class="sticky bottom-0 -mx-5 mt-5 border-t border-slate-100 bg-white px-5 pt-4 dark:border-white/10 dark:bg-[#0D1B2A]">
        ...
        <button type="button" wire:click="order" wire:loading.attr="disabled" wire:target="order"
            ...
            <span wire:loading.remove wire:target="order"><x-icon name="hash" class="mr-1 inline h-4 w-4" /> {{ __('numbers.rent.rent_this_number') }}</span>
        ...
        <p class="mt-2 text-center text-[11px] text-slate-400">{{ __('numbers.rent.wallet_note') }}</p>
    </div>
@endif
