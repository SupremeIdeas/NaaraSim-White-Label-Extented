<div x-data="{ tab: 'mobile' }">
    ...
    <div class="mb-4 inline-flex w-full rounded-full border border-slate-200 bg-slate-100 p-1 dark:border-white/10 dark:bg-white/5">
        <button type="button" @click="tab = 'mobile'" :class="tab === 'mobile' ? 'bg-white text-primary shadow-sm dark:bg-[#243352] dark:text-teal-300' : 'text-slate-500 dark:text-slate-400'" class="flex-1 rounded-full px-4 py-1.5 text-sm font-semibold transition">Mobile numbers</button>
        <button type="button" @click="tab = 'toll'" :class="tab === 'toll' ? 'bg-white text-primary shadow-sm dark:bg-[#243352] dark:text-teal-300' : 'text-slate-500 dark:text-slate-400'" class="flex-1 rounded-full px-4 py-1.5 text-sm font-semibold transition">Toll-free</button>
    </div>
    ...
    @unless ($permanentAvailable)
        <div class="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-slate-300 py-10 text-center dark:border-white/10">
            ...
            <p class="font-semibold text-slate-800 dark:text-slate-100">Naara Line is coming soon</p>
            <p class="max-w-xs text-sm text-slate-500 dark:text-slate-400">A permanent international number with voice &amp; SMS — we’re finishing the last checks with our carrier.</p>
        </div>
    @else
        <p class="mb-3 text-sm text-slate-500 dark:text-slate-400">A permanent international number that’s yours to keep — <b>voice calls and SMS</b>, billed monthly.</p>
        ...
        <div class="space-y-3">
            ...
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
                <label class="mb-1.5 block text-[11px] uppercase tracking-wide text-slate-400">Find a memorable number (optional)</label>
                <div class="relative">
                    <x-icon name="search" class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <input type="text" wire:model="vanity" inputmode="numeric" placeholder="e.g. 777 or ends with 0000"
        ...
        @if ($lineDone)
            <div class="mt-4 rounded-2xl border border-green-200 bg-green-50 p-4 text-center dark:border-green-900/50 dark:bg-green-950/30">
                ...
                <p class="font-bold text-slate-900 dark:text-white">Naara Line active</p>
                <p class="mt-0.5 select-all font-mono text-lg font-bold text-primary dark:text-teal-300">{{ $lineDone }}</p>
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Set up forwarding or the dialer from your dashboard.</p>
            </div>
        @endif
                ...
                @foreach ($lineNumbers as $n)
                    <div wire:key="line-{{ $n['number'] }}" class="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-white/5">
                        <div class="min-w-0">
                            ...
                            <p class="text-[11px] text-slate-400">{{ $n['locality'] ?: 'Mobile' }} · ${{ number_format($n['monthly_retail'], 2) }}/mo</p>
                        </div>
                        <button type="button" wire:click="getLine('{{ $n['number'] }}')" wire:loading.attr="disabled" wire:target="getLine('{{ $n['number'] }}')"
                            ...
                            <span wire:loading.remove wire:target="getLine('{{ $n['number'] }}')">Get this number</span>
        ...
        <div class="mt-4 flex items-start gap-2 rounded-xl bg-primary/5 p-3 text-xs text-slate-500 dark:bg-teal-500/5 dark:text-slate-400">
            ...
            <span>Naara Line is a monthly subscription — the first month is charged now, then it renews monthly. Voice &amp; SMS included.</span>
        </div>
        ...
        <div class="sticky bottom-0 -mx-5 mt-5 border-t border-slate-100 bg-white px-5 pt-4 dark:border-white/10 dark:bg-[#0D1B2A]">
            <button type="button" wire:click="searchLine" wire:loading.attr="disabled" wire:target="searchLine"
                ...
                <span wire:loading.remove wire:target="searchLine"><x-icon name="search" class="mr-1 inline h-4 w-4" /> {{ ! empty($lineNumbers) ? 'Search again' : 'Search available numbers' }}</span>
                <span wire:loading wire:target="searchLine" class="inline-flex items-center gap-2"><x-ui.spinner class="h-4 w-4" /> Searching…</span>
            </button>
        </div>
    @endunless
</div>
